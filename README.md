...
2. **Simple, tool-friendly HTTP API**  
   The Worker exposes a minimal, action-friendly API with just a few endpoints under `/docs`. This keeps integration easy for tools like ChatGPT Actions, scripts, or other services.
...
- **Client** (e.g. ChatGPT Custom GPT, CLI script, other service)  
  Calls the Worker’s `/docs` endpoints with JSON payloads and an Authorization header.
...
- `dir` is optional; when omitted, lists the root docs directory.
- Returns names and paths (files and subdirectories).

### Get a document

```http
GET /docs/ftl/canon.md
```
...
Requests to `/docs` without a valid token receive `401 Unauthorized`.  
The health check (`GET /`) remains unauthenticated for ease of monitoring.
...
- `openapi.yaml` – OpenAPI schema describing the `/docs` API surface (suitable for use as a ChatGPT Action definition).  
...
- **List root docs directory:**
  ...
  ```bash
  ./scripts/call-docstore.sh GET "/docs"
  ```

- **Create or update a document:**

  ```bash
  ./scripts/call-docstore.sh PUT "/docs/ftl/canon.md"         '{"content":"# FTL Canon\n\nHello from the docstore.","message":"Create initial canon"}'
  ```

- **Get that document:**

  ```bash
  ./scripts/call-docstore.sh GET "/docs/ftl/canon.md"
  ```

- **Delete that document:**

  ```bash
  ./scripts/call-docstore.sh DELETE "/docs/ftl/canon.md"         '{"message":"Remove test doc"}'
  ```
